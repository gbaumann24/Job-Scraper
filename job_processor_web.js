...
async function processJobs(db) {
	...
	for (const job of jobs) {
		...
		// 7. Falls nicht-Code Tools vorhanden sind, sende einen zweiten Prompt, um die zugehörigen Website-Adressen zu ermitteln.
		if (extractedData.tools.some((tool) => !tool.code)) {
			const nonCodeTools = extractedData.tools.filter((tool) => !tool.code);
			const toolNames = nonCodeTools.map((tool) => tool.name);
			const websitesPrompt =
				'Dies ist eine Liste von Software-Tools (ohne Programmier-Sprachen):\n' +
				toolNames.map((name) => `- ${name}`).join('\n') +
				'\n\nFür jedes dieser Tools, finde die offizielle Website-Adresse. Überprüfe streng, ob die gefundene Website tatsächlich zur offiziellen Seite des Tools gehört. ' +
				'Gib als Ausgabe ein JSON-Objekt im folgenden Format zurück: {"tools": [{"name": "<Toolname>", "website": "<Website-Adresse>", "valid": <true/false>}, ...]}. ' +
				'Falls keine Website gefunden wird, setze "website" auf "" und "valid" auf false.';
			console.log(`[${new Date().toISOString()}] Zweiter Prompt für Website-Ermittlung erstellt.`);

			let openaiWebsiteResult;
			try {
				console.log(`[${new Date().toISOString()}] Sende Anfrage an OpenAI API für Website-Ermittlung für Job ${job_id} ...`);
				const websiteResponse = await fetch('https://litellm.sph-prod.ethz.ch/v1/chat/completions', {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
						Authorization: `Bearer ${OPENAI_API_KEY}`,
				...
				openaiWebsiteResult = websiteJson.choices && websiteJson.choices[0] && websiteJson.choices[0].message && websiteJson.choices[0].message.content;
				console.log(`[${new Date().toISOString()}] OpenAI API Antwort für Website-Ermittlung erhalten.`);
			} catch (err) {
				console.error(`[${new Date().toISOString()}] Fehler beim Aufruf der OpenAI API für Website-Ermittlung für Job ${job_id}:`, err);
			}

			if (openaiWebsiteResult) {
				let cleanedWebsiteString = openaiWebsiteResult
					.replace(/```(javascript)?\s*/gi, '')
					.replace(/```/g, '')
					.trim();
				if (cleanedWebsiteString.toLowerCase().startsWith('json')) {
					cleanedWebsiteString = cleanedWebsiteString.replace(/^json\s*/i, '');
				}
				console.log(`[${new Date().toISOString()}] OpenAI Website-Ermittlung Antwort bereinigt:\n${cleanedWebsiteString}`);
				let websiteData;
				try {
					websiteData = JSON.parse(cleanedWebsiteString);
					// Validierung: Es muss ein Objekt mit "tools" sein, das ein Array von Objekten enthält,
					// in denen "name" (String), "website" (String) und "valid" (Boolean) definiert sind.
					if (typeof websiteData !== 'object' || websiteData === null || !('tools' in websiteData) || !Array.isArray(websiteData.tools)) {
						throw new Error('Output-Struktur ungültig: Fehlende erforderliche Schlüssel.');
					}
					for (const tool of websiteData.tools) {
						if (
							typeof tool !== 'object' ||
							tool === null ||
							!('name' in tool) ||
							...
							throw new Error('Output-Struktur ungültig: Jedes Tool muss ein Objekt mit "name", "website" (String) und "valid" (Boolean) sein.');
						}
					}
					console.log(`[${new Date().toISOString()}] Validierte Website-Daten für Job ${job_id}: ${JSON.stringify(websiteData)}`);
					// Integriere die Website-Daten in die extrahierten Tools für nicht-Code Tools
					for (const tool of extractedData.tools) {
						if (!tool.code) {
							const websiteEntry = websiteData.tools.find((entry) => entry.name.toLowerCase() === tool.name.toLowerCase());
							if (websiteEntry) {
								tool.website = websiteEntry.website;
								tool.valid = websiteEntry.valid;
							} else {
								tool.website = '';
								tool.valid = false;
							}
					...
					console.error(`[${new Date().toISOString()}] Ungültige Website-Ausgabe für Job ${job_id}:`, err.message);
				}
			}
		}

		// 8. Konvertiere die validierten Daten (inklusive Website-Informationen, falls vorhanden) in einen String zur Speicherung in der Datenbank.
		...
		// 9. Speichere die validierten Tools in der neuen Tabelle
		try {
